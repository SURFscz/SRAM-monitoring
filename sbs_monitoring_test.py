...
try:
    ...
    idp = browser.find_element_by_xpath("//div[@class='text-truncate label primary' and text()='Selenium Test IdP']").click()

    wait.until(staleness_of(search))

    # Login as user
    browser.find_element_by_id('username').send_keys(user)
    browser.find_element_by_id('password').send_keys(password)
    browser.find_element_by_name('f').submit()
